...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const screenDancers = document.getElementById('screen-dancers');
    const btnBackHub = document.getElementById('btn-back-hub');

    // Navigation function
    function switchScreen(targetScreen) {
        [screenHub, screenImages, screenVideos, screenDancers].forEach(s => s && s.classList.add('hidden'));
        if (targetScreen) targetScreen.classList.remove('hidden');

        if (targetScreen === screenDancers) {
            studioApp?.classList.add('fullwidth-mode');
        } else {
            studioApp?.classList.remove('fullwidth-mode');
        }
    ...
    const cardOpenDancers = document.getElementById('card-open-dancers');
    if (cardOpenDancers) {
        cardOpenDancers.addEventListener('click', () => {
            if (tg?.HapticFeedback) tg.HapticFeedback.impactOccurred('medium');
            switchScreen(screenDancers);
        });
    }
    ...
    // Check URL parameters for direct navigation (e.g. ?tab=dancers)
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('tab') === 'dancers') {
        switchScreen(screenDancers);
    }
